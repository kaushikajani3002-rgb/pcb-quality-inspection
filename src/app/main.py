...
# Custom CSS Injector for High-Contrast AOI Theme & Visible Text Labels
st.markdown("""
...
<style>
    ...
    /* FORCE HIGH CONTRAST DARK TEXT FOR ALL SIDEBAR LABELS & WIDGETS */
    ...
    /* File Uploader Container High-Contrast Fix */
    ...
    /* Sidebar Radio Interactive Navigation Items */
...
# -----------------------------------------------------------------------------
# SIDEBAR CONTROLS & FUNCTIONAL CLICKABLE STATION ROUTING
# -----------------------------------------------------------------------------
with st.sidebar:
    st.markdown("""
    <div style="background-color: #006194; border-radius: 8px; padding: 14px; text-align: center; margin-bottom: 16px; color: #ffffff;">
        <span style="font-family: 'Space Grotesk', sans-serif; font-size: 16px; font-weight: 700; letter-spacing: 0.05em; color: #ffffff;">🔬 AOI OPTICAL INSPECTION</span><br>
        <span style="font-family: 'JetBrains Mono', monospace; font-size: 10px; color: #cce5ff;">SMT LINE 02 // STATION 04</span>
    </div>
    """, unsafe_allow_html=True)
    
    # 1. Clickable Station Routing Menu
    station_route = st.radio(
        ...
        help="Click any item to switch station workstation console view."
    )
    ...
    st.markdown("<hr style='border: 0; border-top: 1px solid #bfc7d2; margin: 12px 0;'>", unsafe_allow_html=True)
    st.markdown("<div style='font-family: JetBrains Mono; font-size: 11px; font-weight: 700; color: #131b2e; text-transform: uppercase;'>INFERENCE ENGINES STATUS</div>", unsafe_allow_html=True)
    ...
    st.markdown("<hr style='border: 0; border-top: 1px solid #bfc7d2; margin: 12px 0;'>", unsafe_allow_html=True)
    st.markdown("<div style='font-family: JetBrains Mono; font-size: 11px; font-weight: 700; color: #131b2e; text-transform: uppercase;'>SIMULATION & FLAGS</div>", unsafe_allow_html=True)
    ...
    <div style="background-color: #f2f3ff; border: 1px solid #bfc7d2; border-radius: 6px; padding: 8px 12px; margin-top: 12px; display: flex; align-items: center; justify-content: space-between;">
        <span style="font-family: 'JetBrains Mono', monospace; font-size: 11px; font-weight: 700; color: #131b2e;">PLC CONVEYOR</span>
...
# -----------------------------------------------------------------------------
# DYNAMIC VIEW ROUTING BASED ON CLICKABLE STATION NAVIGATION
# -----------------------------------------------------------------------------
...
# Render Top Industrial Header Strip across all views
st.markdown(f"""
...
# -----------------------------------------------------------------------------
# VIEW 1: INSPECTION CONSOLE (PRIMARY WORKSTATION)
# -----------------------------------------------------------------------------
if station_route == "🔍 Inspection Console":
    st.markdown("<h3 style='font-family: Space Grotesk; font-weight: 700; text-transform: uppercase; margin-bottom: 12px; color: #131b2e;'>📸 Image Acquisition & Dual Optical Channels</h3>", unsafe_allow_html=True)
    ...
    # Execute inspection if RUN button clicked
    if run_clicked:
    ...
    if st.session_state.workflow_status == STATE_COMPLETED:
        ...
        # Aggregated System Status Banner
        st.markdown("<h3 style='font-family: Space Grotesk; font-weight: 700; text-transform: uppercase; color: #131b2e;'>⚡ Aggregated System Inspection Status</h3>", unsafe_allow_html=True)
        ...
        with col_vis1:
            st.markdown("<h4 style='font-family: Space Grotesk; font-weight: 700; margin-bottom: 8px; color: #131b2e;'>COMPONENT OPTICAL RETICLE (CH-01)</h4>", unsafe_allow_html=True)
            if comp_status in ("PASS", "DETECTION_COMPLETE"):
        ...
        with col_vis2:
            st.markdown("<h4 style='font-family: Space Grotesk; font-weight: 700; margin-bottom: 8px; color: #131b2e;'>CIRCUIT TELECENTRIC RETICLE (CH-02)</h4>", unsafe_allow_html=True)
        ...
        st.markdown("<h3 style='font-family: Space Grotesk; font-weight: 700; text-transform: uppercase; color: #131b2e;'>📋 Metrology Verification Registers</h3>", unsafe_allow_html=True)
        ...
        with tab_comp_reg:
            if comp_status in ("PASS", "DETECTION_COMPLETE"):
                ...
                with col_inv_left:
                    st.markdown("<h4 style='font-family: Space Grotesk; font-weight: 700; color: #131b2e;'>TYPE BREAKDOWN SUMMARY</h4>", unsafe_allow_html=True)
                    if detected_counts:
                        summary_rows = []
                        running_total = 0
                ...
                with col_inv_right:
                    st.markdown("<h4 style='font-family: Space Grotesk; font-weight: 700; color: #131b2e;'>ANNOTATED SPATIAL OVERLAY</h4>", unsafe_allow_html=True)
                ...
                st.markdown("---")
                st.markdown("<h4 style='font-family: Space Grotesk; font-weight: 700; color: #131b2e;'>COMPONENT METROLOGY DETECTIONS TABLE</h4>", unsafe_allow_html=True)
                if detected_comps:
        ...
        # Industrial Exporter Action Bar
        st.markdown("<h3 style='font-family: Space Grotesk; font-weight: 700; text-transform: uppercase; margin-top: 24px; color: #131b2e;'>📤 Export Inspection Logs & QC Certificates</h3>", unsafe_allow_html=True)
        ...
        if debug_mode:
            ...
            st.markdown("<h3 style='font-family: Space Grotesk; font-weight: 700; color: #131b2e;'>🛠️ Inference Telemetry & Debug Console</h3>", unsafe_allow_html=True)
...
# -----------------------------------------------------------------------------
# VIEW 2: DEFECT ANALYTICS & PARETO
# -----------------------------------------------------------------------------
elif station_route == "📊 Defect Analytics & Pareto":
    st.markdown("<h3 style='font-family: Space Grotesk; font-weight: 700; text-transform: uppercase; color: #131b2e;'>📊 Defect Analytics & Pareto Distribution</h3>", unsafe_allow_html=True)
    
    col_p1, col_p2, col_p3 = st.columns(3)
    with col_p1:
        st.markdown("""
        <div class="metric-kpi-card">
            <div class="metric-kpi-lbl">TOTAL INSPECTED LOTS</div>
            <div class="metric-kpi-val" style="color: #006194;">1,420</div>
        </div>
        """, unsafe_allow_html=True)
    with col_p2:
        st.markdown("""
        <div class="metric-kpi-card">
            <div class="metric-kpi-lbl">FIRST PASS YIELD (FPY)</div>
            <div class="metric-kpi-val" style="color: #006c49;">97.8%</div>
        </div>
        """, unsafe_allow_html=True)
    with col_p3:
        st.markdown("""
        <div class="metric-kpi-card">
            <div class="metric-kpi-lbl">DEFECT PPO (PARTS PER OVERALL)</div>
            <div class="metric-kpi-val" style="color: #ba1a1a;">14.2 PPM</div>
        </div>
        """, unsafe_allow_html=True)

    st.markdown("<br>", unsafe_allow_html=True)
    
    # Pareto breakdown table & distribution
    st.markdown("<h4 style='font-family: Space Grotesk; font-weight: 700; color: #131b2e;'>PARETO DEFECT FREQUENCY BREAKDOWN</h4>", unsafe_allow_html=True)
    pareto_data = [
        {"Defect Category": "Solder Bridge / Short", "Count": 42, "Percentage": "42.0%", "Cumulative %": "42.0%", "Action Priority": "HIGH"},
        {"Defect Category": "Open Circuit / Trace Fracture", "Count": 28, "Percentage": "28.0%", "Cumulative %": "70.0%", "Action Priority": "HIGH"},
        {"Defect Category": "Copper Spur / Excessive Copper", "Count": 14, "Percentage": "14.0%", "Cumulative %": "84.0%", "Action Priority": "MEDIUM"},
        {"Defect Category": "Mousebite Defect", "Count": 10, "Percentage": "10.0%", "Cumulative %": "94.0%", "Action Priority": "MEDIUM"},
        {"Defect Category": "Missing Hole / Pin Hole", "Count": 6, "Percentage": "6.0%", "Cumulative %": "100.0%", "Action Priority": "LOW"}
    ]
    st.dataframe(pd.DataFrame(pareto_data), use_container_width=True, hide_index=True)

# -----------------------------------------------------------------------------
# VIEW 3: LIVE TELEMETRY & CAMERAS
# -----------------------------------------------------------------------------
elif station_route == "🎥 Live Telemetry & Cameras":
    st.markdown("<h3 style='font-family: Space Grotesk; font-weight: 700; text-transform: uppercase; color: #131b2e;'>🎥 Live Camera Optics & Sensor Telemetry</h3>", unsafe_allow_html=True)
    
    col_t1, col_t2 = st.columns(2)
    with col_t1:
        st.markdown("""
        <div class="aoi-card">
            <h4 style="font-family: 'Space Grotesk'; font-weight: 700; color: #006194;">CH-01 OPTICAL RGB CAMERA SENSOR</h4>
            <ul style="font-family: 'JetBrains Mono', monospace; font-size: 12px; line-height: 1.8;">
                <li><b>Sensor Model:</b> Sony IMX264 CMOS (Global Shutter)</li>
                <li><b>Resolution:</b> 2048 × 1536 (3.1 MP)</li>
                <li><b>Frame Rate:</b> 60.0 FPS Active</li>
                <li><b>Illumination:</b> 5500K Coaxial White LED Ring</li>
                <li><b>Exposure Time:</b> 1,250 μs</li>
                <li><b>Gain Control:</b> 0.0 dB (Calibrated)</li>
            </ul>
        </div>
        """, unsafe_allow_html=True)
    with col_t2:
        st.markdown("""
        <div class="aoi-card">
            <h4 style="font-family: 'Space Grotesk'; font-weight: 700; color: #006c49;">CH-02 TELECENTRIC SOLDER MASK SENSOR</h4>
            <ul style="font-family: 'JetBrains Mono', monospace; font-size: 12px; line-height: 1.8;">
                <li><b>Sensor Model:</b> Teledyne DALSA High-Contrast IR</li>
                <li><b>Resolution:</b> 2048 × 1536 (Polarized)</li>
                <li><b>Frame Rate:</b> 60.0 FPS Active</li>
                <li><b>Illumination:</b> 850nm IR Narrowband Ring Light</li>
                <li><b>Exposure Time:</b> 850 μs</li>
                <li><b>CUDA Inference Latency:</b> 12.8ms</li>
            </ul>
        </div>
        """, unsafe_allow_html=True)

# -----------------------------------------------------------------------------
# VIEW 4: RECIPE & THRESHOLD PARAMS
# -----------------------------------------------------------------------------
elif station_route == "🎛️ Recipe & Threshold Params":
    st.markdown("<h3 style='font-family: Space Grotesk; font-weight: 700; text-transform: uppercase; color: #131b2e;'>🎛️ Recipe & Threshold Parameters Configurator</h3>", unsafe_allow_html=True)
    
    st.markdown(f"**Selected Device Profile:** `{selected_device_lbl}`")
    
    recipe_table = [
        {"Parameter": "Confidence Gate Threshold", "Current Value": f"{conf_threshold:.2f}", "Default": "0.50", "Unit": "Score (0.0-1.0)"},
        {"Parameter": "IoU Overlap NMS Gate", "Current Value": f"{iou_threshold:.2f}", "Default": "0.45", "Unit": "Ratio (0.0-1.0)"},
        {"Parameter": "Positional Misalignment Limit", "Current Value": f"{position_tolerance:.2f}", "Default": "1.50", "Unit": "mm"},
        {"Parameter": "Circuit Defect Detector Model", "Current Value": selected_defect_model_name, "Default": "DeepPCB", "Unit": "YOLO Weights"}
    ]
    st.dataframe(pd.DataFrame(recipe_table), use_container_width=True, hide_index=True)

# -----------------------------------------------------------------------------
# VIEW 5: CALIBRATION & OPTICS
# -----------------------------------------------------------------------------
elif station_route == "🎯 Calibration & Optics":
    st.markdown("<h3 style='font-family: Space Grotesk; font-weight: 700; text-transform: uppercase; color: #131b2e;'>🎯 Telecentric Optics & Spatial Calibration Matrix</h3>", unsafe_allow_html=True)
    
    col_c1, col_c2 = st.columns(2)
    with col_c1:
        st.markdown("""
        <div class="aoi-card">
            <h4 style="font-family: 'Space Grotesk'; font-weight: 700;">OPTICAL MAGNIFICATION SPECS</h4>
            <p><b>Telecentric Lens Factor:</b> 1.000× 1:1 Parallax-Free</p>
            <p><b>Pixel Resolution:</b> 1.000 px = 33.5 μm</p>
            <p><b>Field of View (FOV):</b> 68.6 mm × 53.4 mm</p>
            <p><b>Optical Distortion:</b> < 0.02% (Sub-micron precision)</p>
        </div>
        """, unsafe_allow_html=True)
    with col_c2:
        st.markdown("""
        <div class="aoi-card">
            <h4 style="font-family: 'Space Grotesk'; font-weight: 700;">AFFINE SPATIAL TRANSFORM MATRIX</h4>
            <pre style="font-family: 'JetBrains Mono', monospace; font-size: 11px;">
[ 0.03350  0.00000  0.00000 ]
[ 0.00000  0.03350  0.00000 ]
[ 0.00000  0.00000  1.00000 ]
            </pre>
            <p style="color: #006c49; font-weight: bold;">✓ Matrix Calibrated (IPC-A-610G Standard Passed)</p>
        </div>
        """, unsafe_allow_html=True)

# -----------------------------------------------------------------------------
# VIEW 6: AUDIT TRAIL & EXPORT
# -----------------------------------------------------------------------------
elif station_route == "📜 Audit Trail & Export":
    st.markdown("<h3 style='font-family: Space Grotesk; font-weight: 700; text-transform: uppercase; color: #131b2e;'>📜 Station Audit Trail & QC Inspection Log</h3>", unsafe_allow_html=True)
    
    audit_logs = [
        {"Timestamp": Helper.get_current_timestamp(), "Operator": operator_name, "Station": "SMT-LINE-02", "Action": "Inspection Console Loaded", "Status": "INFO"},
        {"Timestamp": Helper.get_current_timestamp(), "Operator": operator_name, "Station": "SMT-LINE-02", "Action": f"Profile Selected: {selected_device_lbl}", "Status": "SUCCESS"},
        {"Timestamp": Helper.get_current_timestamp(), "Operator": operator_name, "Station": "SMT-LINE-02", "Action": "YOLO Edge Models Loaded & Armed", "Status": "SUCCESS"}
    ]
